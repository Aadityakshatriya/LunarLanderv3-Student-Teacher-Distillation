...
class DistillPPO(PPO):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.last_distill_kl: Optional[float] = None
    ...
    def _kl_teacher_student(self, obs: th.Tensor) -> th.Tensor:
        ...
        if isinstance(teacher_dist, Categorical) and isinstance(student_dist, Categorical):
            t = float(self.distill.temperature)
            # compute KL on temperature-scaled logits
            teacher_logits = teacher_dist.logits / t
            student_logits = student_dist.logits / t
            teacher_log_probs = th.log_softmax(teacher_logits, dim=-1)
            student_log_probs = th.log_softmax(student_logits, dim=-1)
            teacher_probs = th.softmax(teacher_logits, dim=-1)
            kl = th.sum(teacher_probs * (teacher_log_probs - student_log_probs), dim=-1)
            return kl.mean() * (t * t)
    ...
    def train(self) -> None:  # noqa: C901
        ...
        # Log (SB3 logger)
        self.logger.record(
            "train/entropy_loss", float(th.tensor(entropy_losses).mean()) if entropy_losses else 0.0
        )
        self.logger.record(
            "train/policy_gradient_loss", float(th.tensor(pg_losses).mean()) if pg_losses else 0.0
        )
        self.logger.record(
            "train/value_loss", float(th.tensor(value_losses).mean()) if value_losses else 0.0
        )
        self.logger.record(
            "train/approx_kl", float(th.tensor(approx_kls).mean()) if approx_kls else 0.0
        )
        self.logger.record(
            "train/clip_fraction", float(th.tensor(clip_fractions).mean()) if clip_fractions else 0.0
        )
        self.logger.record("train/explained_variance", float(explained_var))
        self.logger.record(
            "train/distill_kl", float(th.tensor(kl_losses).mean()) if kl_losses else 0.0
        )
        self.logger.record("train/distill_alpha", float(self.distill.alpha))
        self.logger.record("train/distill_temperature", float(self.distill.temperature))
        ...
        self.last_distill_kl = float(th.tensor(kl_losses).mean()) if kl_losses else 0.0
